...
export class FixedEntryServer {
  ...
  constructor(
    ...
    this.server = createServer((request, response) => {
      ...
      response.on('error', () => { /* owned by the pipe teardown below */ })
    })
    ...
    this.server.on('upgrade', (request, socket, head) => {
      ...
      socket.on('error', () => { /* torn down by forwardUpgrade */ })
    })
  ...
  private get sessionFresh(): boolean {
   ...
   */
  private rewrite(inbound: IncomingHttpHeaders, cookie: string | null): IncomingHttpHeaders {
  ...
  private async currentCookie(): Promise<string | null> {
   ...
   */
  private async forwardUpgrade(request: IncomingMessage, socket: Duplex, head: Buffer): Promise<void> {
 ...
 */
function rawHeaderText(rawHeaders: readonly string[]): string {
  let text = ''
